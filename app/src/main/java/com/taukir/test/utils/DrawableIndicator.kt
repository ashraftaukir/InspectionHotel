...
class DrawableIndicator @JvmOverloads constructor(
    ...
    private var hasSingleIndicator = false
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        defaultIndex = if (hasSingleIndicator) {
            0
        } else {
            1
        }
        if (getPageSize() > defaultIndex && mCheckedBitmap != null && mNormalBitmap != null) {
            for (i in 0 until getPageSize()) {
                ...
                when {
                    ...
                    i == getCurrentPosition() -> {
                        ...
                        left =
                            (i) * mIndicatorPadding + (i - 1) * mNormalBitmapWidth + mCheckedBitmapWidth
                        top = measuredHeight / 2 - mNormalBitmapHeight / 2
                    }
                }
    ...
    private fun drawIcon(
        canvas: Canvas, left: Int, top: Int, icon: Bitmap?
    ) {
        if (icon == null) {
            return
        }
    ...
    fun enableSingleIndicator() {
        hasSingleIndicator = true
    }

    fun setIndicatorDrawable(
        @DrawableRes normalDrawable: Int, @DrawableRes checkedDrawable: Int
    ): DrawableIndicator {
    ...
    fun setIndicatorSize(
        normalWidth: Int, normalHeight: Int, checkedWidth: Int, checkedHeight: Int
    ): DrawableIndicator {
    ...
    internal class IndicatorSize(
        var normalWidth: Int, var normalHeight: Int, var checkedWidth: Int, var checkedHeight: Int
    )
